...
- Use **Flexbox (`flex`)** for intrinsic, content-driven layouts:
  - 1D layouts (row or column)
  - Alignment (center, space-between, etc.)
  - Navbars, toolbars, buttons, slip rows, card internals
  - Use when elements size themselves naturally

- Use **Grid (`grid`)** for rigid, structured layouts:
  - 2D layouts (rows + columns)
  - Page structure, dashboards, match listings, card grids
  - Use when layout needs strict column/row control

- Always use `gap-*` for spacing between children:
  - Avoid `space-x-*` and `space-y-*` unless necessary

- Prefer `gap` over manual margins:
  - Use `gap-2 / gap-4 / gap-6` based on density
  - Avoid manual spacing unless there’s a visual exception

- Use `flex-wrap` for overflow content (chips, badges, odds rows)

- Default alignment for flex rows:
  - `items-center` + `justify-between`

- Avoid deep flex nesting (max 3 levels)

- Do not mix layout systems unnecessarily:
  - Only combine `grid` and `flex` when there is a clear structural reason
